...
#include <iostream>
#include <vector>
#include "robin-hood/src/include/robin_hood.h"
...
template<>
struct hash<robin_hood::pair<long, long>> {
...
template<>
struct hash<robin_hood::pair<long, int>> {
...
void translate_sequence(std::string& sequence ){
     ...
     const std::unordered_map<char, char> translation_table{
	     {'H','N'}, {'D', 'N'}, {'K', 'N'}, {'M', 'N'}, {'R', 'N'}, {'S', 'N'}, {'W', 'N'}, {'Y', 'N'}, {'a', 'A'}, {'c', 'C'}, {'t', 'T'}, {'g', 'G'}, {'n', 'N'}
     };
    for (char& c : sequence) {
...
std::string get_reversed_strand(const std::string_view& s) {
    ...
    // Iterate over the input string and append the complement character to the output string
    for (char i : s) {
                ...
                for (int i = 0; i < k; i++) {
                    if (pos - i >= 0)[[likely]] {
                        ...
                        for (auto map : kmers[abs(kmer_index)]) {
                                auto new_pos = map + ((kmer_index^data[map]) < 0) * (k - 1) + 
 ...
 std::vector<char>& states)
{
    ...
    else if( (states[abs(choped[i])]-1)/7!=(state-1)/7 && states[abs(choped[i])]%7!=state%7)
    {
...
template<typename T1, typename T2>
void process_block( robin_hood::unordered_node_map<robin_hood::pair<T2, T2>, T2>&  nodes_id, std::vector<std::pair<T1,T1>>& nodes_labels,
std::vector<T2>& path,T1 start, T1 stop, T2& x, const std::string& sequence, const std::vector<T2>& transformed)
{   //std::cout<<transformed[start]<<" "<< transformed[stop]<< std::endl;
...
template<typename T1, typename T2>
void collapse_paths(const std::vector<char>& states,const std::vector<T2>& transformed, const std::string& sequence,
 std::vector<std::pair<T1,T1>>& nodes_labels, std::vector<std::vector<T2>>& paths){
    ...
    if ((states[abs(transformed[i])]==1 && transformed[i]>0) || (states[abs(transformed[i])]==2 && transformed[i]<0)) // BRANCH ON LEFT
    {   if (i==start){continue ;}
        //std::cout<<1<<std::endl;
        //std::cout<<start<<std::endl;
        process_block(nodes_id, nodes_labels, path, start, i-1, x, sequence,transformed);
        start=i;

    }

    else if ((states[abs(transformed[i])]==1 && transformed[i]<0) || (states[abs(transformed[i])]==2 && transformed[i]>0)) // BRANCH ON RIGHT
    {   //std::cout<<2<<std::endl;
        process_block(nodes_id, nodes_labels, path, start, i, x, sequence,transformed);
        start = i+1;
    }
    ...
    {   //std::cout<<3<<std::endl;
        ...
        process_block(nodes_id, nodes_labels, path, i, i, x, sequence,transformed);
        start = i+1;
    ...
    if (transformed[i]==0) // ADD LAST BLOCK 
            {
                if (start<i){std::cout<<"TO: "<<i-1<<std::endl;
...
template<typename T1, typename T2>
void write_gfa(const std::vector<std::pair<T1,T1>>&labels, const std::vector<std::vector<T2>>& paths, const std::string& filename,const std::string& sequence, const std::vector<std::string>& names)
    {   
	...
	std::ofstream output;
        ...
        for (size_t i=0; i<labels.size();i++)
            {
                output<<"S\t"<<i+1<<"\t"<<sequence.substr(labels[i].first,labels[i].second)<<"\n";
            }
	    ...
	    if ((edges.find(edge)!=edges.end())||(edges.find(reversed_edge)!=edges.end())){continue;}
	    edges.insert(edge);	    
	    char sign_left = (path[i]>0)? '+':'-';
            ...
            output<<"L\t"<<left<<"\t"<<sign_left<<"\t"<<right<<"\t"<<sign_right<<"\tOM\n";
        }}
...
int main(int argc, char *argv[]) {
   ...
   // Apply translation table to each sequence
    ...
    if(total_length<INT_MAX){
        ...
        find_connected_bfs(kmers_vec, sequences, kmers_pos, k, choped, choped_nodes_number);
        kmers_vec.clear();
        for(auto v: kmers_pos){v.clear();}
        kmers_pos.clear();
